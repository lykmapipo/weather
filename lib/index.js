 ...
 * @ignore
 */
const CITIES = lodash.map(KNOWN_CITIES, (knownCity) => {
  // obtain and normalize city data
  const [cityId, city] = knownCity.split(':');

  // compact and return city data
  return common.mergeObjects({ city, cityId: lodash.toNumber(cityId) });
});
...
const findCity = (optns) => {
  // ensure options
  const { name } = common.mergeObjects(optns);

  // ignore
  if (common.isNotValue(name)) {
    return undefined;
  }

  // lookup given city from well known cities
  const cityName = lodash.trim(name);
  const foundCity = lodash.find(CITIES, ({ city, cityId }) => {
    return (
...
const parseTime = (date, time, separator = ':') => {
  // ensure date & time
  if (common.isNotValue(date) || common.isNotValue(time)) {
    return undefined;
  }

  // create base date moment
  let baseMoment = moment.utc(date);
  if (common.isNotValue(baseMoment.valueOf())) {
    return undefined;
  }

  // obtain hour and minutes from time string
  const [hours, minutes] = time.split(separator);
  if (common.isNotValue(hours) || common.isNotValue(minutes)) {
    return undefined;
  }
  ...
  // derive date time
  const derivedDate = common.isNotValue(baseMoment.valueOf())
    ? undefined
    : baseMoment.toDate();
...
const normalizePresentForecast = (forecast, city) => {
  ...
  const { present } = common.mergeObjects(forecast);
  const presentForecasts = common.compact([].concat(lodash.values(present)));

  // ensure present city
  const presentCity = common.mergeObjects(city);

  // find city present forecast
  let presentForecast = lodash.find(presentForecasts, (cityForecast) => {
    return String(cityForecast.cityId) === String(presentCity.cityId);
  });

  // normalize & convert
  const date = common.parseDate(lodash.get(presentForecast, 'issue'), 'YYYYMMDD');
  presentForecast = common.mergeObjects(presentCity, {
    date,
    issuedAt: common.parseDate(lodash.get(presentForecast, 'issue'), 'YYYYMMDDHH'),
    weather: lodash.get(presentForecast, 'wxdesc'),
    temperature: lodash.toNumber(lodash.get(presentForecast, 'temp')), //= > °C
    relativeHumidity: lodash.toNumber(lodash.get(presentForecast, 'rh')),
    windDirection: lodash.get(presentForecast, 'wd'),
    windSpeed: (lodash.toNumber(lodash.get(presentForecast, 'ws')) * 18) / 5, //= > km/h
    sunRiseAt: parseTime(date, lodash.get(presentForecast, 'sunrise')),
    sunSetAt: parseTime(date, lodash.get(presentForecast, 'sunset')),
    moonRiseAt: parseTime(date, lodash.get(presentForecast, 'moonrise')),
    moonSetAt: parseTime(date, lodash.get(presentForecast, 'moonset')),
    present: true,
  });
...
const normalizeWeekForecasts = (forecasts, city) => {
  ...
  } = common.mergeObjects({ forecast: { forecastDay: [] } }, forecasts);
  const weekForecasts = common.compact([].concat(forecastDay));

  // ensure present city
  const presentCity = common.mergeObjects(city);

  // normalize & convert
  const cityWeekForecasts = lodash.map(weekForecasts, (forecast) => {
    const date = common.parseDate(lodash.get(forecast, 'forecastDate'), 'YYYY-MM-DD');
    return common.mergeObjects(presentCity, {
      date,
      issuedAt: common.parseDate(issueDate, 'YYYY-MM-DD HH:mm:ss'),
      weather: lodash.get(forecast, 'weather'),
      temperature: lodash.toNumber(lodash.get(forecast, 'temp')), //= > °C
      minimumTemperature: lodash.toNumber(lodash.get(forecast, 'minTemp')), //= > °C
      maximumTemperature: lodash.toNumber(lodash.get(forecast, 'maxTemp')), //= > °C
      relativeHumidity: lodash.toNumber(lodash.get(forecast, 'rh')),
      windDirection: lodash.get(forecast, 'wd'),
      windSpeed: (lodash.toNumber(lodash.get(forecast, 'ws')) * 18) / 5, //= > km/h
      sunRiseAt: parseTime(date, lodash.get(forecast, 'sunrise')),
      sunSetAt: parseTime(date, lodash.get(forecast, 'sunset')),
      moonRiseAt: parseTime(date, lodash.get(forecast, 'moonrise')),
      moonSetAt: parseTime(date, lodash.get(forecast, 'moonset')),
      present: false,
    });
  });
...
const fetchPresentForecast = (optns) => {
  // normalize options
  const { city, ...options } = common.mergeObjects(optns, {
    headers: DEFAULT_REQUEST_HEADERS,
  });

  // find city
  const presentCity = findCity({ name: city });
  if (common.isNotValue(presentCity)) {
  ...
  return httpClient.get(url, options).then((forecast) => {
...
const fetchWeekForecasts = (optns) => {
  ...
  return httpClient.get(url, options).then((forecasts) => {
...
const fetchForecasts = (optns) => {
  ...
  return httpClient.all(fetchPresent, fetchWeek).then(
    ([presentForecast, weekForecasts]) => {
      return common.compact([presentForecast, ...weekForecasts]);
    }
  );
};

exports.fetchForecasts = fetchForecasts;
exports.fetchPresentForecast = fetchPresentForecast;
exports.fetchWeekForecasts = fetchWeekForecasts;
