...
const normalizePresentForecast = (forecast, city) => {
  ...
  // normalize & convert
  const date = common.parseDate(
    lodash.get(presentForecast, 'issue'),
    'YYYYMMDD'
  );
  presentForecast = common.mergeObjects(presentCity, {
    date,
    issuedAt: common.parseDate(
      lodash.get(presentForecast, 'issue'),
      'YYYYMMDDHH'
    ),
...
const normalizeWeekForecasts = (forecasts, city) => {
  ...
  const cityWeekForecasts = lodash.map(weekForecasts, (forecast) => {
    const date = common.parseDate(
      lodash.get(forecast, 'forecastDate'),
      'YYYY-MM-DD'
    );
...
const fetchForecasts = (optns) => {
  ...
  return httpClient
    .all(fetchPresent, fetchWeek)
    .then(([presentForecast, weekForecasts]) => {
